function [] = LsysDrawA(LsysString, plotParameters, plotTitle, figNo)
% LsysDraw: Draw a string obtained from a L-system with some parameters and
...
%
%Sample test call: LsysDrawA(LsysString, plotParameters, plotTitle, figNo)
%
...
%Variable to add on the cummulative turnings (for the cases with digits)
turnNo = 0;
...
for i=1:length(LsysString)
    ...
    case {'F', 'G','M','N'}
        %Assign an index for each letter correspondinng to one index of the
        ...
        newxT = xT + plotParameters(j).length*cos(aT);
        newyT = yT + plotParameters(j).length*sin(aT);
