function pltdistribution(dim,boid,prd,pred,arrow,plotFov, zor, fov,k,ind,visionMode)
...
% visionMode = vision mode configuration to plot
%
% OUTPUTS:
...
if plotFov
    ...
    for i=listInd
        ...
        if visionMode(2) == 1
            ...
            if acos(dot(dis,vel)/(norm(dis)*norm(vel))) < fov/2
                % Save the birds that are inside the field of view
                fovInd(fovCount) = i;
                fovCount = fovCount + 1;
    ...
    % Remove the zeros of the array to avoid index problems
    fovInd = fovInd(1:fovCount-1);
    % Plotting of the k-nearest boids
    if visionMode(3) == 1
        % If field of view is also used, only the birds inside of it will
        % be considered to compute the closest ones
        if visionMode(2) == 1
            disXY = boid(fovInd,1:2) - boid(ind,1:2);
            dis = sqrt(disXY(:,1).^2+disXY(:,2).^2);
            [~,I] = sort(dis);
            I = fovInd(I);
            I = I(1:k);
            if k > length(I)
        ...
        % Otherwise all birds will be analyzed 
        else
            disXY = boid(:,1:2) - boid(ind,1:2);
            dis = sqrt(disXY(:,1).^2+disXY(:,2).^2);
            [~,I] = sort(dis);
        ...
        % Plot a downwards pointing triangle for the k nearest boids
        for i=I
            hold on
            scatter(boid(i,1),boid(i,2),...
                    25,'v','filled','MarkerFaceColor','k')
